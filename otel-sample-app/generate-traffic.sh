...
# Generate traffic for OTEL sample app
echo "Starting traffic generation for OTEL sample app..."

# Get service URL
SERVICE_URL="http://localhost:8000"
if kubectl get svc otel-sample-app >/dev/null 2>&1; then
    echo "Using Kubernetes service via port-forward"
    kubectl port-forward svc/otel-sample-app 8000:8000 &
    PORT_FORWARD_PID=$!
    sleep 3
fi
...
while [ $(date +%s) -lt $end_time ]; do
    ...
    make_request "/generate-logs?count=5"
